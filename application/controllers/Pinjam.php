...
class Pinjam extends CI_Controller
{
    ...
    public function bookingDetail()
    {
        ...
        $data['detail'] = $this->db->query("select id_tiket,armada,tujuan_wisata,panitia,jadwal_keberangkatan from booking_detail d, tiket b where d.id_tiket=b.id and d.id_booking='$id_booking'")->result_array();
        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
    ...
    public function pinjamAct()
    {
        ...
        $this->db->query("UPDATE tiket, detail_pinjam SET tiket.dipesan=tiket.dipesan+1, tiket.dibooking=tiket.dibooking-1 WHERE tiket.id=detail_pinjam.id_tiket");
    ...
    public function ubahStatus()
    {
        ...
        $where = ['id_tiket' => $this->uri->segment(3),];
        $tgl = date('Y-m-d');
        $status = 'Kembali';
        //update status menjadi kembali pada saat buku dikembalikan
        $this->db->query("UPDATE pinjam, detail_pinjam SET pinjam.status='$status', pinjam.tgl_pengembalian='$tgl' WHERE detail_pinjam.id_tiket='$id_tiket' AND pinjam.no_pinjam='$no_pinjam'");
        //update stok dan dipinjam pada tabel buku
        $this->db->query("UPDATE tiket, detail_pinjam SET tiket.dipesan=tiket.dipesan-1, tiket.stok=tiket.stok+1 WHERE tiket.id=detail_pinjam.id_tiket");
