...
class ModelBooking extends CI_Model
{
    ...
    public function joinOrder($where)
    {
        ...
        $this->db->join('tiket bu ', 'bu.id=d.id_tiket');
        $this->db->where($where);
        return $this->db->get();
    }
    public function simpanDetail($where = null)
    {
        $sql = "INSERT INTO booking_detail (id_booking,id_tiket) SELECT booking.id_booking,temp.id_tiket FROM booking, temp WHERE temp.id_user=booking.id_user AND booking.id_user='$where'";
        $this->db->query($sql);
    }
    ...
    public function find($where)
    {
        ...
        $this->db->limit(1);
        return $this->db->get('tiket', $where);
    }
    ...
    {
        $this->db->query('CREATE TABLE IF NOT EXISTS temp(id_booking varchar(12), tgl_booking DATETIME, email_user varchar(128), id_tiket int)');
    }
    public function selectJoin()
    {
        ...
        $this->db->join('tiket', 'booking_detail.id_tiket=tiket.id');
        return $this->db->get();
    }
